function [violation_status,margin] = check_violation(mpc,suppress_warning)
    ...
    qg_inj0 = imag(v_cplx0.*conj(Y*v_cplx0))+Cl*qpq0;
    ...
    for i = 1:Nbus
        ...
        if (qg_inj0(i)>qinj_max(i)+limit_tolerence || qg_inj0(i)<qinj_min(i)-limit_tolerence)&& ismember(i,gen(:,GEN_BUS))
    ...
    for i = 1:Ngen
        ...
        if p_dispatch(i)>gen(i,GEN_STATUS)*gen(i,PMAX)+limit_tolerence || p_dispatch(i)<gen(i,GEN_STATUS)*gen(i,PMIN)-limit_tolerence
            ...
            if suppress_warning == 0
                warning(['Pg violation at gen ', num2str(i), ': ', num2str(round(gen(i,PMIN), digits_spec)), ' ≤ ', num2str(round(p_dispatch, digits_spec)), ' ≤ ', num2str(round(gen(i,PMAX), digits_spec))]);
            end
            margin = min(margin, min(gen(i,PMAX) - p_dispatch, p_dispatch - gen(i,PMIN)));
        end
    end
    ...
    for i = 1:Nbranch
        ...
        if angle_diff(i)>(branch(i,ANGMAX)+limit_tolerence) || angle_diff(i)<(branch(i,ANGMIN)-limit_tolerence)
            ...
            margin = min(margin,min(deg2rad(branch(i,ANGMAX))-angle_diff,angle_diff-deg2rad(branch(i,ANGMIN))));
        end
