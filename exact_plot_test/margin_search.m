...
[PW_LINEAR, POLYNOMIAL, MODEL, STARTUP, SHUTDOWN, NCOST, COST] = idx_cost;
    ...
    vmag_slack     =@(x)create_vmag_slack(x(entries_pf{1}),x(entries_pf{5}(1)),vmax,vmin);
    phi_slack      =@(x)create_phi_slack(x(entries_pf{2}),x(entries_pf{5}(2)),C,Phimax,Phimin);
    gen_slack      =@(x)create_gen_slack(x(entries_pf{3}),x(entries_pf{4}),...
        x(entries_pf{5}(3)),x(entries_pf{5}(4)),Pgmax,Pgmin,Qgmax,Qgmin);
    line_slack     =@(x)create_line_slack(x(entries_pf{1}), x(entries_pf{2}),...
        x(entries_pf{5}(5)), Gf, Bf, Gt, Bt, Fmax, from_bus,to_bus);
    margin_cons    =@(x)create_margin(x(entries_pf{5}(1)),x(entries_pf{5}(2)),...
    ...
    lbg = vertcat(-inf(2*Nbus+4*Nbranch+4*Ngen+5,1), zeros(Npf+1,1),Pg0(id_gen_slack),Qg0(id_gen_slack));
    ubg = vertcat(zeros(2*Nbus+4*Nbranch+4*Ngen+5,1),zeros(Npf+1,1),Pg0(id_gen_slack),Qg0(id_gen_slack));
